...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        """
        """*** YOUR CODE HERE ***"""
        control = list(state[1])
        if state[0] in self.corners:
            if state[0] not in state[1]:
                control.append(state[0])

            return len(control) == 4
        return False
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                if (nextx, nexty) in self.corners:
                    if (nextx, nexty) not in control:
                        control.append((nextx, nexty))

                successors.append((((nextx, nexty), tuple(control)), action, 1))
...
def cornersHeuristic(state, problem):
    ...
    for x in corners:
        if x not in state[1]:
            heuristics.append((mazeDistance(state[0],x,problem.game)))
...
def foodHeuristic(state, problem):
    ...
    for x in remFoodList:
        heuristicVals.append(mazeDistance(x, state[0],problem.startingGameState))
